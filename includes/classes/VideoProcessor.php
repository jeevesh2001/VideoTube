...
class VideoProcessor
{
  ...
  private $ffmpegPath = "ffmpeg/linux/ffmpeg -i";
  ...
  {
    $cmd = "$this->ffmpegPath $tempFilePath $finalFilePath 2>&1";
    $outputLog = array();
  ...
  public function generateThumbnails($filePath)
  {
    ...
    for ($num = 1; $num <= $numThumbnails; $num++) {
      ...
      $cmd = "$this->ffmpegPath $filePath -ss $interval -s $thumbnailSize -vframes 1 $fullThumbnailPath 2>&1";
      $outputLog = array();
      exec($cmd, $outputLog, $returnCode);
      if ($returnCode != 0) {
        // command failed
